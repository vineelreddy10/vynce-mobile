...
function LoginForm({ email, setEmail, password, setPassword, error, submitting, handleSubmit }: {
  ...
  return (
    <div className="bg-white rounded-2xl shadow-glow p-6 w-full max-w-sm mx-auto space-y-5">
      <form className="space-y-4" onSubmit={handleSubmit}>
        <div>
          <label htmlFor="email" className="block text-sm font-semibold text-on-surface mb-1.5 font-headline tracking-wider uppercase text-xs">
            Email or Username
          </label>
        ...
        <div>
          <label htmlFor="password" className="block text-sm font-semibold text-on-surface mb-1.5 font-headline tracking-wider uppercase text-xs">Password</label>
      ...
      <div className="space-y-2.5">
        <button className="w-full flex items-center justify-center gap-2 border border-border rounded-full py-2.5 text-sm font-medium text-on-surface hover:bg-muted transition-colors">
      ...
      <p className="text-center text-sm text-muted-foreground mt-2">Don't have an account? <Link to="/register" className="text-primary font-semibold hover:underline">Create one</Link></p>
      ...
      <div className="lg:hidden flex flex-col">
        ...
        <div className="flex-1 px-5 -mt-6 relative z-10">
          <LoginForm email={email} setEmail={setEmail} password={password} setPassword={setPassword}
            error={error} submitting={submitting} handleSubmit={handleSubmit} />
        </div>
      </div>
